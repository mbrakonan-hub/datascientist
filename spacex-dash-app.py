...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='class',
        title=f"Success pie chart for {entered_site}") 
        return fig
...
def get_scatter_chart(entered_site, payload_range):
    ...
    df_filtered = df_filtered[
        (df_filtered['Payload Mass (kg)'] >= low) &
        (df_filtered['Payload Mass (kg)'] <= high)
    ]
    ...
    fig1 = px.scatter(
        ...
        title=title,
        hover_data=['class', "Payload Mass (kg)", "Booster Version Category"]
    )
